...
app.get('/test-api-key', async (_req, res) => {
  try {
    ...
    if (response.ok) {
      ...
      const hasRealtimeModel = data.data?.some((m: any) => 
        m.id === 'gpt-realtime-mini-2025-10-06' || 
        m.id === 'gpt-4o-realtime-preview' ||
      ...
      res.json({ 
        ...
        hasRealtimeAccess: hasRealtimeModel,
        availableRealtimeModels: data.data?.filter((m: any) => m.id.includes('realtime')).map((m: any) => m.id)
      })
    } else {
...
wss.on('connection', (clientWs: WSWebSocket, req: IncomingMessage) => {
  ...
  // Try without model in URL - will be set in session.update instead
  const openaiUrl = OPENAI_API_URL
  log.info(`Connecting to OpenAI: ${sessionId}`, { 
    url: openaiUrl,
    model: model  // Log model separately for debugging
  })
  ...
  try {
    ...
    const openaiWs = new WSWebSocket(openaiUrl, 'realtime', {
      headers: {
        Authorization: `Bearer ${OPENAI_API_KEY}`,
        'OpenAI-Beta': 'realtime=v1'  // Add beta header for compatibility
      }
    })
