<template>
  <div class="landing-language-selector">
    <!-- Language Button -->
    <button 
          ...
          <div class="modal-content" @click.stop>
            ...
            <div class="language-grid">
              ...
              >
                ...
                <i v-if="selectedLanguage.code === lang.code" class="pi pi-check checkmark" />
              </button>
            </div>
          </div>
        </div>
...
import { AVAILABLE_LANGUAGES, type Language } from '@/stores/language'
import { isValidLanguage, saveLanguagePreference } from '@/router/languageRouting'
...
const selectedLanguage = computed(() => {
  ...
  if (urlLang) {
    const found = AVAILABLE_LANGUAGES.find(l => l.code === urlLang)
    if (found) return found
  }
  // Try to match from i18n locale
  const found = AVAILABLE_LANGUAGES.find(l => l.code === locale.value)
  return found || AVAILABLE_LANGUAGES[0] // Default to English
})
...
watch(() => route.params.lang, (newLang) => {
  if (newLang && isValidLanguage(newLang as string)) {
...
function selectLanguage(language: Language) {
  ...
  // Save preference (cast to LanguageCode since we know AVAILABLE_LANGUAGES contains valid codes)
  ...
  if (currentLang && currentLang !== language.code) {
    ...
    const newPath = route.fullPath.replace(`/${currentLang}/`, `/${language.code}/`)
      .replace(`/${currentLang}`, `/${language.code}`)
...
/* Language Grid - 5 columns for 10 languages */
.language-grid {
  padding: 1.5rem;
  display: grid;
