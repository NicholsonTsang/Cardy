    ...
    <div class="px-6 py-20 md:px-12 lg:px-20 flex items-center justify-center bg-surface-50 dark:bg-surface-950 min-h-screen">
        <div class="max-w-xl w-full flex flex-col items-start gap-8 bg-surface-0 dark:bg-surface-900 p-12 rounded-3xl shadow-[0px_24px_48px_0px_rgba(0,0,0,0.04)]">
            <div class="flex flex-col items-center gap-6 w-full">
                ...
                <h1 class="text-center text-2xl font-medium text-surface-900 dark:text-surface-0 leading-tight w-full">Welcome Back</h1>
            </div>
            <div class="flex items-center gap-4 w-full">
                <Button @click="handleGoogleSignIn" :disabled="isLoading" outlined icon="pi pi-google !text-base !leading-none" severity="secondary" class="!flex-1 !py-2 !text-surface-900 dark:!text-surface-0" label="Google" />
                ...
                <!-- <Button outlined icon="pi pi-apple !text-base !leading-none" severity="secondary" class="!flex-1 !py-2 !text-surface-900 dark:!text-surface-0" /> -->
            </div>
            ...
            <form @submit.prevent="handleSignIn" class="flex flex-col gap-6 w-full">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-3 sm:gap-0">
                    ...
                    <a class="text-surface-500 dark:text-surface-400 font-medium cursor-pointer hover:text-surface-600 dark:hover:text-surface-300">Forgot your password?</a>
                </div>
                <div class="flex flex-col gap-10 w-full">
                    <Button type="submit" label="Sign In" class="w-full" :loading="isLoading" />
                    <div class="text-center w-full">
                        <span class="text-surface-900 dark:text-surface-0 font-medium">Don't have an account? </span>
                        <a @click="goToSignUp" class="text-primary font-medium cursor-pointer hover:text-primary-emphasis">Create today!</a>
                    </div>
                </div>
            </form>
        </div>
...
async function handleSignIn() {
  ...
  try {
    ...
    // Navigation is handled by the auth store's onAuthStateChange or signInWithEmail method
  } catch (error) {
...
async function handleGoogleSignIn() {
  ...
  try {
    ...
    // It's okay as the page will navigate away
  }
}
